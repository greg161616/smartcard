...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Title — visible when sidebar is OPEN */
    #headerTitle {
    ...
    .sidebar-toggle:hover { background: rgba(0,0,0,0.38); }
    ...
    .profile-circle:hover { border-color: #fff; }
    ...
    .admin-sidebar.ready { transition: transform .50s ease; }
    .admin-sidebar.open  { transform: translateX(0); }

    /* Sidebar topbar — same height & color as main header */
    .sidebar-topbar {
      height: var(--header-height);
      background: var(--header-bg);
      display: flex;
    ...
    .sidebar-close-btn:hover { background : rgba(0,0,0,0.38); }
  ...
  <script>
    (function() {
      window.__sidebarInitOpen = localStorage.getItem('sidebarOpen') === 'true';
    })();
  </script>
</head>
<body>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Set initial state without animation to prevent flash on load
  applyState(isOpen, false);
  requestAnimationFrame(() => requestAnimationFrame(() => sidebar.classList.add('ready')));

  function openSidebar()  { isOpen = true;  localStorage.setItem('sidebarOpen', true);  applyState(true,  true); }
  function closeSidebar() { isOpen = false; localStorage.setItem('sidebarOpen', false); applyState(false, true); }
