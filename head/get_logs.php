...
function getLogStatistics($conn) {
    ...
    // User activities
    $result = $conn->query("SELECT COUNT(*) as total FROM system_logs WHERE log_type = 'user'");
    $stats['userActivities'] = $result->fetch_assoc()['total'];
    
    // Grade updates
    $result = $conn->query("SELECT COUNT(*) as total FROM system_logs WHERE log_type = 'grade'");
    $stats['gradeUpdates'] = $result->fetch_assoc()['total'];
    
    // Today's logs
    $result = $conn->query("SELECT COUNT(*) as total FROM system_logs WHERE DATE(timestamp) = CURDATE()");
...
function clearOldLogs($conn) {
    ...
    $stmt = $conn->prepare("DELETE FROM system_logs WHERE timestamp < ?");
